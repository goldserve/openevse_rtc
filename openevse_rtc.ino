 ...
 * Open EVSE Firmware
 *
 * Copyright (c) 2011-2012 Sam C. Lin <lincomatic@gmail.com> and Chris Howell <chris1howell@msn.com> and Kevin L <Paypal goldserve1@hotmail.com>
 * Maintainers: SCL/CH
...
#include <Wire.h>
#include "RTClib.h"      // Required for RTC and Delay Timer
...
public:
  ...
  void flush() { 
    Serial.flush();
    // Fix to clear command line buffer - GoldServe
    memset(m_CLIinstr,'\0',CLI_BUFLEN); //clear buffer
  }
  void getInput();
  uint8_t getInt();
};
...
// Changed from default 0x00 to 0x4A for testing - GoldServe
...
{ 
  ...
  delay(1000);
  uint8_t CustomChar_0[8] = {0x0,0xe,0x15,0x17,0x11,0xe,0x0,0x0};
  uint8_t CustomChar_1[8] = {0x0,0x0,0xe,0xe,0xe,0x0,0x0,0x0};
  uint8_t CustomChar_2[8] = {0x0,0x8,0xc,0xe,0xc,0x8,0x0,0x0};
  m_Lcd.createChar(0, CustomChar_0);
  m_Lcd.createChar(1, CustomChar_1);
  m_Lcd.createChar(2, CustomChar_2);
#endif //#ifdef LCD16X2
#endif //#ifdef DELAYTIMER
...
void OnboardDisplay::Update()
{
  ...
  // Display a new stopped LCD screen with Delay Timers enabled - GoldServe
    ...
    if (g_DelayTimer.IsTimerEnabled()){
      ...
      g_OBD.LcdWrite(0x2);
      g_OBD.LcdWrite(0x0);
      sprintf(g_sTmp,"%02d:%02d",g_DelayTimer.GetStartTimerHour(),g_DelayTimer.GetStartTimerMin());
      LcdPrint(11,0,g_sTmp);
      g_OBD.LcdSetCursor(9,1);
      g_OBD.LcdWrite(0x1);
      g_OBD.LcdWrite(0x0);
      sprintf(g_sTmp,"%02d:%02d",g_DelayTimer.GetStopTimerHour(),g_DelayTimer.GetStopTimerMin());
...
void DelayMenuStartHour::Init()
{
  ...
  m_CurIdx = g_hour;
  sprintf(g_sTmp,"+%02d:%02d", m_CurIdx, g_min);
  g_OBD.LcdPrint(1,g_sTmp);
}
...
void DelayMenuStartMin::Init()
{
  ...
  m_CurIdx = g_min;
  sprintf(g_sTmp,"%02d:+%02d", g_hour, m_CurIdx);
  g_OBD.LcdPrint(1,g_sTmp);
}
...
void DelayTimer::Init(){
  ...
  if (rtmp == 0xff) { // uninitialized EEPROM
    m_StartTimerHour = ECF_DEFAULT_START_HOUR;
    EEPROM.write(EOFS_TIMER_START_HOUR, m_StartTimerHour);
  }
  else {
  ...
  if (rtmp == 0xff) { // uninitialized EEPROM
    m_StartTimerMin = ECF_DEFAULT_START_MIN;
    EEPROM.write(EOFS_TIMER_START_MIN, m_StartTimerMin);
  }
  else {
  ...
  if (rtmp == 0xff) { // uninitialized EEPROM
    m_StopTimerHour = ECF_DEFAULT_STOP_HOUR;
    EEPROM.write(EOFS_TIMER_STOP_HOUR, m_StopTimerHour);
  }
  else {
  ...
  if (rtmp == 0xff) { // uninitialized EEPROM
    m_StopTimerMin = ECF_DEFAULT_STOP_MIN;
    EEPROM.write(EOFS_TIMER_STOP_MIN, m_StopTimerMin);
  }
  else {
    m_StopTimerMin = rtmp;
